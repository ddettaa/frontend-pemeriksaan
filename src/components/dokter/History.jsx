...
const History = () => {
  ...
  // const [isFocused, setIsFocused] = useState(false); // unused, removed for lint clean
  ...
  const fetchPatients = useCallback(async () => {
    try {
      ...
      if (data && Array.isArray(data.data)) {
        ...
        // Filter by tanggal dan status selesai diperiksa
        filteredPatients = filteredPatients.filter(
          (patient) => {
                      ...
                      <tr key={patient.no_registrasi} className="hover:bg-gray-50 text-gray-700 transition-colors duration-200">
                        ...
                        <td className="px-2 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            (patient.status === 2 || (typeof patient.status === "string" && patient.status.toLowerCase() === "selesai diperiksa"))
                        ...
                        <td className="px-4 py-3 flex gap-2">
                          {console.log('DEBUG patient row:', patient)}
                          <Link to={`/dokter/rangkuman/${patient.no_registrasi}`} title="Lihat rangkuman pemeriksaan">
                            <button
                              className="text-[#0099a8] hover:text-[#007a85] p-1.5 rounded-full transition-colors duration-200 bg-transparent border border-[#0099a8] hover:border-[#007a85]"
                              onClick={e => {
                                e.stopPropagation();
                                console.log('DEBUG klik aksi:', patient);
                              }}
                            >
